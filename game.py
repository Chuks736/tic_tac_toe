...
class TicTacToe:
    def __init__(self):
        self.board = [' ' for _ in range(9)]  # we will use a single list to rap 3x3 board
    ...
    def winner(self, square, letter):
        ...
        # check column
        col_ind = square % 3
        column = [self.board[col_ind] for i in range(3)]
        if all([spot == letter for spot in column]):
            return True

        # check diagonal
        # but only if the square is an even number [0, 2, 4, 6, 8]
        # these are the ony moves possible to win a diagonal
        if square % 2 == 0:
